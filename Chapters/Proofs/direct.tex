...
% \lipsum[1]
Direct proofs, are well, direct. They take a claim and then prove it by steps that build one of off another arriving at a desired conclusion.
\begin{definition}\quad
    \begin{enumerate}
        \item The sum of integers is an integer
        \item the difference of integers is an integer
        \item the product of integers is an integer
        \item Every integer is even or odd
    \end{enumerate}
\end{definition}
\begin{prop}
...
\begin{proof}
        $m$ and $n$ are even numbers. This means $m=2a$ and $n=2b$ for some integers\\ $a$ and $b$.
        \begin{align*}
                m+n=2a+2b&\text{ Add both equations.}\\
                m+n=2(a+b)&\text{ Factor out 2.}
            \end{align*}
        Since $a+b$ is some other integer, we have shown $m+n=2k$ where $k=a+b$.
\end{proof}

How to write a proof? Here is a basic layout, the more you do, the more intuitive they become.
\begin{theorem}
    Here goes a theorem.
    \lipsum[1]
\end{theorem}
